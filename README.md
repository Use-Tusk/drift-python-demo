...
# Tusk Drift Demo - Python/Flask Service
...
This demo includes a simple Flask API service that integrates with external APIs. You'll:
...
git clone https://github.com/Use-Tusk/drift-python-demo.git
cd drift-python-demo
...
- **Flask Server** (`server.py`) - API service with multiple external integrations using Requests library
...
Want to see Tusk catch a bug? Open this [new codespace](https://codespaces.new/Use-Tusk/drift-python-demo/tree/buggy-branch) or switch to `buggy-branch` in the repo:
...
Changes to `server.py` in `buggy-branch`:
...
@@ -24,6 +28,8 @@ def weather_activity():
     )
...
[See full list of supported packages →](https://github.com/Use-Tusk/drift-python-sdk#requirements)
...
```bash
git checkout main
TUSK_DRIFT_MODE=record python server.py
```

This sets `TUSK_DRIFT_MODE=record` environment variable and starts the server.
...
Minimal changes - just initialize the SDK at startup and call `tusk_drift.mark_app_as_ready()` when your app is ready to accept requests.
